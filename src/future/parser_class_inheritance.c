...
#define __PARSER_CLASS_INHERITANCE_SOURCE__
 ...
 ***************************************************************/
globle PACKED_CLASS_LINKS *ParseSuperclasses(void *theEnv, char *readSource, ATOM_HN *newClassName)
{
    ...
    {
        error_syntax(theEnv, "defclass inheritance");
        return(NULL);
    }
    ...
    if((core_get_type(DefclassData(theEnv)->ObjectParseToken) != ATOM) ? TRUE :
       (DefclassData(theEnv)->ObjectParseToken.value != (void *)DefclassData(theEnv)->ISA_ATOM))
    {
        error_syntax(theEnv, "defclass inheritance");
        return(NULL);
    }
    ...
    while( core_get_type(DefclassData(theEnv)->ObjectParseToken) != RPAREN )
    {
        ...
        if( find_module_separator(to_string(newClassName)))
        {
            error_illegal_module_specifier(theEnv);
            goto SuperclassParseError;
        }

        if( core_get_value(DefclassData(theEnv)->ObjectParseToken) == (void *)newClassName )
        {
            error_print_id(theEnv, "INHERPSR", 1, FALSE);
            print_router(theEnv, WERROR, "A class may not have itself as a superclass.\n");
            goto SuperclassParseError;
        }

        for( ctmp = clink ; ctmp != NULL ; ctmp = ctmp->nxt )
        {
            if( core_get_value(DefclassData(theEnv)->ObjectParseToken) == (void *)ctmp->cls->header.name )
            {
                error_print_id(theEnv, "INHERPSR", 2, FALSE);
                print_router(theEnv, WERROR, "A class may inherit from a superclass only once.\n");
                goto SuperclassParseError;
            }
        }

        sclass = LookupDefclassInScope(theEnv, to_string(core_get_value(DefclassData(theEnv)->ObjectParseToken)));

        if( sclass == NULL )
        {
            error_print_id(theEnv, "INHERPSR", 3, FALSE);
            print_router(theEnv, WERROR, "A class must be defined after all its superclasses.\n");
            goto SuperclassParseError;
        }
        ...
        {
            error_print_id(theEnv, "INHERPSR", 6, FALSE);
            print_router(theEnv, WERROR, "A user-defined class cannot be a subclass of ");
            print_router(theEnv, WERROR, EnvGetDefclassName(theEnv, (void *)sclass));
    ...
    if( clink == NULL )
    {
        error_print_id(theEnv, "INHERPSR", 4, FALSE);
        print_router(theEnv, WERROR, "Must have at least one superclass.\n");
        return(NULL);
    }
    ...
    core_backup_pp_buffer(theEnv);
    core_save_pp_buffer(theEnv, ")");
    plinks = core_mem_get_struct(theEnv, packedClassLinks);
...
globle PACKED_CLASS_LINKS *FindPrecedenceList(void *theEnv, DEFCLASS *cls, PACKED_CLASS_LINKS *supers)
{
    ...
    if( cls != NULL )
    {
        pop = core_mem_get_struct(theEnv, partialOrder);
        pop->cls = cls;
        pop->pre = 0;
        pop->suc = NULL;
        pop->nxt = po_table;
    ...
    while( pop != start )
    {
        ...
        if( pop->pre == 0 )
        {
            ...
            while( pop->suc != NULL )
            {
                ...
                pop->suc = stmp->nxt;
                stmp->po->pre--;
                core_mem_return_struct(theEnv, successor, stmp);
            }
            ...
            pop = pop->nxt;
            start = poprv;
            ptmp = core_mem_get_struct(theEnv, classLink);
            ptmp->cls = potmp->cls;
            ptmp->nxt = NULL;
            core_mem_return_struct(theEnv, partialOrder, potmp);
    ...
    if( po_table != NULL )
    {
        error_print_id(theEnv, "INHERPSR", 5, FALSE);
...
globle void PackClassLinks(void *theEnv, PACKED_CLASS_LINKS *plinks, CLASS_LINK *lptop)
{
    ...
    if( count > 0 )
    {
        plinks->classArray = (DEFCLASS **)core_mem_alloc_no_init(theEnv, (sizeof(DEFCLASS *) * count));
    }
    else
    {
 ...
 *  NOTES        : None
 **************************************************************************/
static PARTIAL_ORDER *InitializePartialOrderTable(void *theEnv, PARTIAL_ORDER *po_table, PACKED_CLASS_LINKS *supers)
{
 ...
 *  NOTES        : None
 ***********************************************************************************/
static void RecordPartialOrders(void *theEnv, PARTIAL_ORDER *po_table, DEFCLASS *cls, PACKED_CLASS_LINKS *successors, long starti)
{
    ...
    {
        stmp = core_mem_get_struct(theEnv, successor);
 ...
 *  NOTES        : None
 ***************************************************/
static PARTIAL_ORDER *FindPartialOrder(PARTIAL_ORDER *po_table, DEFCLASS *cls)
{
    while( po_table != NULL )
    {
        if( po_table->cls == cls )
 ...
 **************************************************************************/
static void PrintPartialOrderLoop(void *theEnv, PARTIAL_ORDER *po_table)
{
    ...
    print_router(theEnv, WERROR, "Precedence loop in superclasses:");
 ...
 *  NOTES        : None
 ***************************************************/
static void PrintClassLinks(void *theEnv, char *logical_name, char *title, CLASS_LINK *clink)
{
    if( title != NULL )
    {
    ...
    while( clink != NULL )
    {
        ...
        PrintClassName(theEnv, logical_name, clink->cls, FALSE);
        clink = clink->nxt;
    }
